...
const dbProduit = new sqlite3.Database('./database.sqlite', (err) => {
    if (err) {
        ...
        dbProduit.run('CREATE TABLE IF NOT EXISTS produit (id INTEGER PRIMARY KEY AUTOINCREMENT, titre TEXT NOT NULL, description TEXT NOT NULL, prix INT NOT NULL)', (err) => {
            if (err) {
